  ...
  class Packager::PKG < Packager::Base
    ...
    def sign_software_libs_and_bins
      if signing_identity
        ...
        signed_shas = {}
        log.info(log_key) { "  Signing libraries:" } if !libraries.empty?
        libraries.each do |library|
          log.debug(log_key) { "    Signing: #{library}" }
          sign_library(library)
        end

        log.info(log_key) { "  Signing binaries:" } if !binaries.empty?
        binaries.each do |binary|
          log.debug(log_key) { "    Signing: #{binary}" }
          sign_binary(binary, true)
        end
        return libraries.merge binaries
      end
    end
    ...
    def is_macho?(lib)
      if is_binary?(lib)
        ...
        stdout = shellout!(command).stdout
        stdout.match?(/Mach-O.*library/) || stdout.match?(/Mach-O.*bundle/)
      else
        false
      end
    end
